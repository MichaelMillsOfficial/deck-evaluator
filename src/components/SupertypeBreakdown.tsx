...
import { computeSupertypeBreakdown } from "@/lib/supertypes";

interface SupertypeBreakdownProps {
  deck: DeckData;
...
export default function SupertypeBreakdown({
  deck,
  cardMap,
}: SupertypeBreakdownProps) {
  ...
  const sortedTypes = useMemo(() => {
    const breakdown = computeSupertypeBreakdown(allCardNames, cardMap);
  ...
  return (
    <div data-testid="supertype-breakdown">
      <p className="mb-3 text-xs text-slate-400">
        Supertype frequency across the deck
      </p>
        ...
        {sortedTypes.map(([type, count]) => (
          <div key={type} className="flex items-center gap-2">
            ...
            <div className="relative h-5 flex-1 rounded bg-slate-700">
              <div
                className={`absolute inset-y-0 left-0 rounded ${BAR_COLORS[type] ?? "bg-slate-500/60"}`}
