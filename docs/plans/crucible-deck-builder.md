...
- **Shipped as fast-follows (post-v1):** share-a-pile URL (`/crucible?p=` link plus a Forge-style `.dck` download, with a matching `.dck` file import that parses back into a fresh pile; see `src/lib/crucible-share.ts`); a deck-name input at seal on the tracker rail (blank falls back to the default name "Forged in the Crucible"); true row virtualization via `@tanstack/react-virtual` (replaces the v1 per-group render cap and "Show all N" expansion). The `?p=` share and `.dck` import decoders are hardened against untrusted input (gzip-bomb output cap, pool/name/quantity bounds, upload size cap) in `src/lib/crucible-share.ts`.
